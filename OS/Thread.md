...
|  | 멀티 프로세스 | 멀티 스레드 |
| --- | --- | --- |
| 메모리 영역 공유 | 각 프로세스는 독립적인 메모리 영역을 가짐. | Code, Data, Heap 영역 공유 (Stack 영역은 독립적)  |
| 컨텍스트 스위칭 | 비용이 크다.  | 비교적 비용이 작다. |
| 동기화  | 프로세스 간 통신을 위해 공유 메모리를 사용할 때, 데이터의 일관성 유지를 위해 동기화가 필요함. | 스레드는 기본적으로 프로세스의 메모리 영역을 일부 공유하므로, 해당 영역의 데이터에 접근할 때 동기화가 필요함.  |
| 예외 전파 | 한 프로세스가 종료되어도 다른 프로세스에는 영향을 미치지 않음.  | 한 스레드가 종료되면 다른 스레드도 종료될 수 있음.  |
