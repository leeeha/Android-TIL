...
운영체제가 스레드를 관리하고 문맥 전환을 수행하는 데 필수적인 정보들을 담고 있는 자료구조 (스레드 ID, 레지스터 집합, 프로그램 카운터, 스택 포인터 등)
...
- 장점
  ...
  - **확장성**: 멀티 프로세서 (또는 멀티 코어) 환경에서 각 스레드는 병렬적으로 수행된다. 
- 단점
...
- 경쟁 조건: 여러 스레드가 동일한 자원에 접근하여 값을 변경하려고 할 때, 데이터의 일관성을 잃을 수 있으므로 동기화 처리가 필요하다. 
- 데드락: 여러 스레드가 서로의 자원을 기다리며 무한 대기 상태에 빠질 수 있다. 
- 기아 현상: 특정 스레드가 우선순위에 밀려 계속해서 자원을 얻지 못하는 기아 현상이 발생할 수 있다. 
- 컨텍스트 스위칭 오버헤드: 프로세스보다 가볍더라도 스레드 간의 전환이 너무 잦으면, 성능 저하가 발생할 수 있다. 
...
- **불변 객체** 
  - 공유 자원을 불변 객체로 만들면 한번 생성한 후에 값을 변경할 수 없어서 안전하다.
