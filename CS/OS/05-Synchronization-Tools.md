...
위의 그림은 프로세스의 자원 할당 상태를 나타내는 자원 할당 그래프이다. 동그라미는 프로세스, 네모는 자원, 네모 안의 점은 해당 자원의 인스턴스를 의미한다.

- 프로세스 → 자원: 프로세스가 해당 자원을 요청하는 것 
- 프로세스 ← 자원: 프로세스가 해당 자원을 할당받은 것 

자원 할당 그래프로 데드락의 발생을 탐지하는 방법은 다음과 같다. 
...
- 프로세스의 자원 요청에 대한 부가적인 정보를 이용해 **데드락 발생 가능성이 없는 경우에만 자원을 할당**한다. 
...
세마포어(semaphore: 신호 장치, 신호기)는 **프로세스 동기화 기능을 편리하게 제공하는 추상 데이터 타입**이다. 
...
싱글 코어 내에서 여러 개의 프로세스 또는 스레드가 번갈아가며 실행되어 마치 동시에 실행되는 것처럼 보이는 것 
...
멀티 코어 내에서 여러 개의 프로세스 또는 스레드가 실제로 동시에 실행되는 것 
...
여러 프로세스가 공유 메모리 내의 동일한 자원에 동시에 접근하면, 명령어의 실행 순서에 따라 비일관적인 결과가 나올 수 있는데, 이를 경쟁 상태라고 부른다. 

이를 해결하기 위한 방법이 프로세스 동기화이며, 공유 데이터에는 한번에 단 하나의 프로세스만 접근할 수 있게 만든다. 
...
임계 구역에서는 오직 단 하나의 프로세스만 임계 자원에 접근하여 값을 읽거나 변경할 수 있다. 
...
여러 프로세스가 동일한 자원에 대해 서로 할당받기 위해 경쟁하고 있는 상황 
...
운영체제가 지원하는 소프트웨어 솔루션으로는 뮤텍스, 세마포어가 있고, 자바 같은 고수준 언어에서는 모니터를 제공한다.

하드웨어 솔루션으로는 원자성을 보장하는 test_and_set() 명령어가 있다. 
...
공유 자원에 대한 접근을 lock으로 제어하여 상호 배제를 보장하는 기법 (이진 세마포어와 유사)
...
공유 자원에 대한 프로세스 동기화 기능을 편리하게 제공하는 추상 데이터 타입 

</details>
<br>

<details>
<summary>뮤텍스(Mutex)와 이진 세마포어의 차이에 대해 설명해주세요.</summary>
...
뮤텍스, 세마포어로 프로세스를 직접 동기화 할 때의 문제점을 해결하기 위해, 프로그래밍 언어 단에서 제공하는 추상 데이터 타입 
...
여러 프로세스가 서로의 자원을 요청하며 무한정 대기하는 상태 
