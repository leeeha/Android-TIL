...
- 두 연산은 원자적으로 수행되어야 한다. (하드웨어 지원으로 가능)
...
- 종류로는 S가 0과 1만 왔다갔다 하는 이진 세마포어, S를 사용 가능한 리소스 개수로 설정하는 카운팅 세마포어가 있다. 

</details>
<br>

<details>
<summary>뮤텍스와 세마포어는 각각 언제 사용하는 게 좋을까요?</summary>

- 뮤텍스
  - 공유 자원에 단 하나의 스레드만 접근하도록 상호 배제를 보장하고 싶을 때 
  - 예시: 데이터베이스 또는 파일 입출력 작업 시 하나의 스레드만 접근 
- 세마포어 
  - 공유 자원에 동시 접근 가능한 스레드 개수를 제한하고 싶을 때
  - 예시: 데이터베이스 연결 풀, 스레드 풀 크기를 제한하여 여러 클라이언트의 요청을 동시에 처리 

여기서 드는 의문은, 세마포어 변수가 1보다 크면 여러 스레드가 공유 자원에 동시 접근하여 데이터 비일관성 문제가 발생하지 않는가? 

그렇다. 이 문제를 해결하기 위해 다음과 같이 스레드를 동기화 할 수 있다. 

- 세마포어와 뮤텍스의 조합: 세마포어로 공유 자원에 접근 가능한 스레드 개수를 제한하되, 임계 구역의 코드를 실행시킬 때는 뮤텍스를 사용해 상호 배제 보장 
- 이진 세마포어 활용: 세마포어 변수가 1이면 뮤텍스와 동일하므로 이를 활용 
- 각 스레드의 작업 범위 분리: 각 스레드가 공유 자원에서 접근 가능한 데이터 범위를 분리하여 상호 배제 보장 
