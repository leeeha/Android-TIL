...
<details>
<summary>CPU burst time 측정 방법은?</summary>

정확한 시간은 미리 알 수 없다. 따라서, 과거의 CPU burst time을 통해 미래의 CPU burst time을 예측하는 방식으로 구한다.
...
(CPU 사용률, 처리량은 최대화 ↔️ 처리 시간, 대기 시간, 응답 시간 최소화)
...
- 장기 스케줄러: 디스크에 있는 프로세스 중에 어떤 프로세스를 준비 큐로 보낼지 결정 
...
- 준비 큐를 여러 개의 레벨로 분류하여 스케줄링 하는 방식 
- 전위 큐에는 응답 시간이 짧아야 하는 대화형 작업, 후위 큐에는 계산 위주의 배치 작업 
- 여러 개의 준비 큐 스케줄링: 고정 우선순위 방식, 타임 슬라이스 방식 
...
Aging 기법을 적용하여 우선순위가 낮은 큐에서 오래 대기한 경우 우선순위가 높은 큐로 이동시키는 방식 (기아 현상 방지)
